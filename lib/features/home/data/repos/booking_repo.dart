...
class BookingUserRepo {
  ...
  Future<UserBookingModel> updateBooking(
      UserBookingModel booking, String book_id) async {
    try {
      String accessToken = await getAccessToken(dio);

      Map<String, dynamic> getWorkSpaceResponce =
          await dio.put(EndPoint.updateBook, accessToken,
              data: {
                'book_id': book_id,
                'price': booking.price,
                'date': booking.date,
