...
class EditBookingCubit extends Cubit<EditBookingState> {
  ...
  //
  //
  void removeClientbooking(BookModel bookModel) async {
    emit(EditBookingLoadingState());
    try {
      final editStatus = await BookingsAdminRepo(dio).RemoveBookings(bookModel);
      getIt<BookingsCubit>().getClientBookings();
      log("hi message success");

      emit(EditBookingSuccessState());
    } on ServerException catch (e) {
      log("hi message error ${e.errorModel.message}");
  ...
  //
  //
  void updateAdmingbooking(BookModel bookModel) async {
    emit(EditBookingLoadingState());
    try {
      final editStatus =
          await BookingsAdminRepo(dio).changeSatusOfBookings(bookModel);
      getIt<BookingsCubit>().getAdminBookings();
      emit(EditBookingSuccessState());
    } on ServerException catch (e) {
      emit(EditBookingFailState(errorMessage: e.errorModel.message));
    }
